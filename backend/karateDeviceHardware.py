...
class Makiwara(object):
    ...
    def update_plot(self, frame, accel, line, fig1, ax3):
        time.sleep(0.05)
        ...
        for acc in new_data:
            ...
            global_vars.x1.append(round(xData * conv_G, 6))
        print(len(global_vars.x1))
        line.set_ydata(global_vars.x1)
        line.set_xdata(range(len(global_vars.x1)))
        ax3.set_xlim(0, len(global_vars.x1))
        return line,
    ...
    def convertData(self, t_end2):
        ...
        try:
            ...
            print(e)
            name = "admin"
        # mock does not need any save
        # df_to_save = global_vars.rawDataFrame.copy()
        # df_to_save['t'] = global_vars.time_vector
        # saveNameCsv = "/home/karate/karateProjectFullstack/data/" + name + 'rawData' + global_vars.time_vector[0].strftime("%y.%m.%d-%H:%M:%S.%f") + '.csv'
        # global_vars.rawDataFrame.to_csv(saveNameCsv, mode='w', header=True, index=True)
